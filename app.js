// GreenTech app - estructura inspirada en arquitectura hexagonal
// Capa de dominio: reglas y entidades del negocio
const domain = {
  ...
  authenticateUser(username, password) {
    return username === this.LOGIN_USERNAME && password === this.LOGIN_PASSWORD;
  },
...
const application = {
  ...
  renderKpis(kpis) {
    document.getElementById('kpi-deforestation').textContent = kpis.deforestation.toLocaleString();
    document.getElementById('kpi-deforestation-change').textContent = kpis.deforestationChange;
    document.getElementById('kpi-deforestation-progress').style.width = kpis.deforestationProgress + '%';
    document.getElementById('kpi-active-alerts').textContent = kpis.activeAlerts;
    document.getElementById('kpi-sensors-online').textContent = kpis.sensorsOnline;
    document.getElementById('kpi-carbon-sequestration').textContent = kpis.carbonSequestration;
  },
...
// Capa de infraestructura: DOM, almacenamiento y librerías externas
const infrastructure = {
  ...
  handleLoginSubmit(event, loginError, loginScreen, pageSidebar, pageMain) {
    event.preventDefault();
    const username = document.getElementById('loginUser')?.value.trim();
    const password = document.getElementById('loginPassword')?.value;
    if (domain.authenticateUser(username, password)) {
  ...
  handleLogout(loginScreen, pageSidebar, pageMain) {
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  function handleLoginSubmit(event) {
    infrastructure.handleLoginSubmit(event, loginError, loginScreen, pageSidebar, pageMain);
  }
  ...
  function init() {
    let state = application.loadState(infrastructure.storage);
    ...
    application.saveState(infrastructure.storage, state);
    ...
    refreshButtons.forEach(refreshButton => {
      refreshButton.addEventListener('click', () => {
        state = application.loadState(infrastructure.storage);
